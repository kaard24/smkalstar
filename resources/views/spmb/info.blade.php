                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-3 sm:gap-6">
                            @forelse($jurusan as $j)
                                ...
                                <a href="{{ url('/jurusan/' . strtolower($j->kode)) }}" class="flex flex-col border {{ $j->warna_border }} rounded-2xl sm:rounded-3xl bg-white hover:shadow-lg transition duration-300 group overflow-hidden cursor-pointer">
                                    ...
                                    <div class="p-2.5 sm:p-3 flex items-center gap-2 sm:gap-3">
                                        <div class="w-8 h-8 sm:w-10 sm:h-10 rounded-full {{ $j->warna_bg }} flex items-center justify-center overflow-hidden flex-shrink-0 ring-2 ring-white shadow-sm {{ $j->warna_hover }} transition">
                                            <img src="{{ $j->logo_url }}" alt="Logo {{ $j->nama }}" class="w-full h-full object-cover">
                                        </div>
                                        <h3 class="font-bold text-gray-900 text-xs sm:text-sm group-hover:{{ $j->warna_text }} transition leading-tight">{{ $j->nama }}</h3>
                                    </div>
                                </a>
                            @empty
                    ...
                    <div>
                        ...
                        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                            <div class="p-4 sm:p-6 md:p-8 grid grid-cols-2 gap-3 sm:gap-4">
                                ...
                                @endphp
                                <div class="flex flex-col justify-between gap-3 p-3 sm:p-4 rounded-2xl {{ $isBerlangsung ? 'bg-sky-50/50 border border-sky-100' : ($isSelesai ? 'bg-gray-50 border border-gray-100' : 'border border-gray-100 hover:bg-gray-50 transition') }}">
                                    <div class="flex items-start gap-3">
                                        <div class="w-9 h-9 sm:w-12 sm:h-12 rounded-full {{ $isBerlangsung ? 'bg-sky-100 text-sky-600' : ($isSelesai ? 'bg-gray-200 text-gray-500' : 'bg-gray-100 text-gray-400') }} flex items-center justify-center flex-shrink-0">
                                            <span class="text-sm sm:text-lg font-bold">{{ $g->nomor }}</span>
                                        </div>
                                        <div class="min-w-0">
                                            <h4 class="font-bold text-gray-900 text-sm sm:text-base leading-tight">{{ $g->nama }}</h4>
                                            <p class="text-xs sm:text-sm text-gray-600 font-medium mt-1 leading-snug">{{ $g->pendaftaran_start->translatedFormat('d M Y') }} - {{ $g->pendaftaran_end->translatedFormat('d M Y') }}</p>
                                        </div>
                                    </div>
                                    @if($isBerlangsung)
