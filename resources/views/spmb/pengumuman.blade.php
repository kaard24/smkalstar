                ...
                <div class="lg:col-span-2">
                    ...
                    <div class="bg-white rounded-2xl md:rounded-3xl shadow-xl overflow-hidden mb-6 md:mb-8 border border-gray-100">
                        <div class="p-5 sm:p-8 md:p-12">
                            <form action="{{ route('spmb.pengumuman.cek') }}" method="GET" class="space-y-4 md:space-y-6">
                                <div>
                        ...
                        <div class="bg-white rounded-2xl md:rounded-3xl shadow-xl overflow-hidden animate-fade-in-up mb-6 md:mb-8">
                            <div class="p-6 sm:p-10 text-center">
                                @if($hasil->status_kelulusan == 'Lulus')
                                    ...
                                    <p class="text-lg sm:text-xl text-gray-900 font-bold mb-2">{{ $hasil->nama }}</p>
                                    ...
                                    <div class="bg-gray-50 rounded-xl sm:rounded-2xl p-4 sm:p-6 mb-6 sm:mb-8 border border-gray-100 inline-block w-full max-w-sm">
                                        ...
                                        <p class="text-xl sm:text-2xl font-bold text-primary">{{ $hasil->jurusan }}</p>
                                    </div>
                ...
                <div class="lg:col-span-1 space-y-4 md:space-y-6">
                    ...
                    <div class="bg-white rounded-2xl md:rounded-3xl p-4 sm:p-6 shadow-lg border border-gray-100">
                        ...
                        <div class="space-y-3 sm:space-y-4">
                            @forelse($statistikJurusan->take(4) as $index => $jurusan)
                                <div class="flex items-center gap-2 sm:gap-3">
                                    <div class="w-7 h-7 sm:w-8 sm:h-8 rounded-lg {{ $index == 0 ? 'bg-yellow-100 text-yellow-600' : ($index == 1 ? 'bg-gray-100 text-gray-600' : ($index == 2 ? 'bg-orange-100 text-orange-600' : 'bg-blue-50 text-blue-500')) }} flex items-center justify-center text-xs sm:text-sm font-bold flex-shrink-0">
                                        {{ $index + 1 }}
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <div class="flex items-center justify-between mb-1">
                                            <span class="text-xs sm:text-sm font-medium text-gray-700 truncate pr-2">{{ $jurusan['nama'] }}</span>
                                            <span class="text-xs sm:text-sm font-bold text-gray-900 flex-shrink-0">{{ $jurusan['total'] }}</span>
                                        </div>
                                        <div class="h-1.5 sm:h-2 bg-gray-100 rounded-full overflow-hidden">
                                            <div class="h-full {{ $index == 0 ? 'bg-yellow-400' : ($index == 1 ? 'bg-gray-400' : ($index == 2 ? 'bg-orange-400' : 'bg-blue-400')) }} rounded-full transition-all duration-500" style="width: {{ $jurusan['persentase'] }}%"></div>
                                        </div>
                                    </div>
                                </div>
                            @empty
    ...
    <section class="py-10 sm:py-16 bg-gradient-to-br from-blue-600 to-cyan-600 relative overflow-hidden">
        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
            ...
            <div class="flex flex-col sm:flex-row flex-wrap justify-center gap-3 sm:gap-4">
                ...
                <a href="{{ route('spmb.info') }}" class="inline-flex items-center justify-center px-6 sm:px-8 py-3 sm:py-4 bg-blue-500/30 text-white font-bold rounded-xl sm:rounded-2xl hover:bg-blue-500/50 transition border border-white/30 text-sm sm:text-base">
                    <svg class="w-4 h-4 sm:w-5 sm:h-5 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                    </svg>
