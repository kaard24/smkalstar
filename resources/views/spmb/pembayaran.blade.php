...
@section('title', 'Pembayaran - SPMB ALSTAR')
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden mb-6">
        <div class="p-6 {{ $currentStatus ? $statusColors[$currentStatus] : 'bg-gray-50 text-gray-700 border-gray-200' }}">
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-lg font-bold">Status Pembayaran</h2>
                    <p class="text-2xl font-bold mt-1">{{ $currentStatus ? $statusLabels[$currentStatus] : 'Belum Bayar' }}</p>
                </div>
                <div class="w-16 h-16 rounded-full bg-white/30 flex items-center justify-center">
                    ...
                    @elseif($currentStatus == Pembayaran::STATUS_PENDING)
                    <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                    </svg>
                    @elseif($currentStatus == Pembayaran::STATUS_VERIFIED)
                    <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @if($currentStatus == Pembayaran::STATUS_REJECTED)
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
            ...
            <div class="space-y-4">
                <div class="p-4 bg-blue-50 rounded-lg border border-blue-100">
                    ...
                    <p class="text-2xl font-bold text-blue-800">Rp {{ number_format($biayaPendaftaran, 0, ',', '.') }}</p>
                </div>
                        ...
                        <div class="p-3 border border-slate-200 rounded-lg">
                            <div class="flex items-start gap-3">
                                <div class="w-10 h-10 bg-slate-100 rounded-lg flex items-center justify-center flex-shrink-0">
                                    <span class="text-xs font-bold text-slate-600">{{ substr($rekening['nama_bank'], 0, 2) }}</span>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <p class="font-medium text-slate-800">{{ $rekening['nama_bank'] }}</p>
                                    <div class="flex items-center gap-2 mt-1">
                                        <p class="text-sm text-slate-600 font-mono bg-slate-50 px-2 py-1 rounded select-all">{{ $rekening['no_rekening'] }}</p>
                                        <button onclick="copyRekening('{{ $rekening['no_rekening'] }}', this)" 
                                    ...
                                    <p class="text-xs text-slate-500 mt-1">a.n. {{ $rekening['atas_nama'] }}</p>
                                </div>
                            </div>
                        </div>
                        @endforeach
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
            ...
            <div class="mb-4">
                ...
                <div class="relative">
                    <img src="{{ route('spmb.pembayaran.preview', $pembayaran->id) }}" alt="Bukti Pembayaran" class="w-full h-48 object-cover rounded-lg border">
            ...
            <form action="{{ route('spmb.pembayaran.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="mb-4">
                    ...
                    <input type="file" name="bukti_pembayaran" accept="image/jpeg,image/png,image/jpg" 
                        class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-[#4276A3] focus:border-[#4276A3] @error('bukti_pembayaran') border-red-500 @enderror"
                        {{ !$pembayaran || $pembayaran->status == Pembayaran::STATUS_REJECTED ? 'required' : '' }}>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="catatan" rows="2" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-[#4276A3] focus:border-[#4276A3]" placeholder="Contoh: Transfer dari Bank BCA">{{ old('catatan', $pembayaran?->catatan_admin) }}</textarea>
                </div>
                ...
                <button type="submit" class="w-full px-4 py-2 bg-[#4276A3] text-white font-medium rounded-lg hover:bg-[#3a6a94] focus:outline-none focus:ring-2 focus:ring-[#4276A3] focus:ring-offset-2">
                    {{ $pembayaran ? 'Upload Ulang Bukti Pembayaran' : 'Upload Bukti Pembayaran' }}
                </button>
            </form>
        </div>
        @else
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
            ...
            <div class="space-y-3">
                <div>
                    ...
                    <p class="font-medium text-slate-800">Rp {{ number_format($pembayaran->jumlah, 0, ',', '.') }}</p>
                </div>
                <div>
                ...
                <div>
                    ...
                    <a href="{{ route('spmb.pembayaran.preview', $pembayaran->id) }}" target="_blank" class="inline-flex items-center text-[#4276A3] hover:underline">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
        ...
        <ol class="space-y-2 text-sm text-slate-600 list-decimal list-inside">
            <li>Transfer biaya pendaftaran sebesar <strong>Rp {{ number_format($biayaPendaftaran, 0, ',', '.') }}</strong> ke salah satu rekening di atas.</li>
