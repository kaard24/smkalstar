...
<div class="max-w-4xl mx-auto">
    <div class="mb-8">
        <h1 class="text-2xl font-bold text-slate-800">Edit Profil Sekolah</h1>
        <p class="text-slate-600">Kelola konten halaman profil sekolah yang ditampilkan ke publik.</p>
    </div>
    ...
    <form action="{{ route('admin.profil-sekolah.update') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Gambar Sejarah</label>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
            ...
            <div class="p-6">
                <textarea id="visi" name="visi" rows="3" required
                    class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-[#4276A3] focus:border-[#4276A3] transition">{{ old('visi', $profil->visi) }}</textarea>
            </div>
        </div>
                ...
                <div class="mb-4 relative inline-block">
                    ...
                    <form action="{{ route('admin.profil-sekolah.delete-struktur') }}" method="POST" class="inline absolute top-2 right-2">
                        ...
                        <button type="submit" onclick="return confirm('Hapus gambar struktur organisasi?')" 
                            class="btn-icon btn-icon-delete btn-icon-sm">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                        </button>
                    </form>
                </div>
                @else
                <div class="bg-slate-50 border-2 border-dashed border-slate-300 rounded-xl p-8 text-center mb-4">
                    <svg class="w-12 h-12 text-gray-300 mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                    <p class="text-slate-500 text-sm">Belum ada gambar struktur organisasi</p>
                </div>
                @endif
                <input type="file" id="struktur_organisasi_gambar" name="struktur_organisasi_gambar" accept="image/*"
                    class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-[#4276A3] focus:border-[#4276A3] transition file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-[#4276A3]/10 file:text-[#4276A3] hover:file:bg-[#4276A3]/20">
                <p class="mt-1 text-xs text-slate-500">Format: JPG, PNG, WebP. Maks. 2MB. Rekomendasi ukuran: 1200x800px.</p>
            </div>
        </div>

        {{-- Submit Button --}}
        <div class="flex justify-end gap-4">
            <a href="{{ url('/profil') }}" target="_blank" class="btn btn-outline-secondary btn-lg">
