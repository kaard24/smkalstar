...
<div class="max-w-6xl mx-auto">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-slate-100 bg-slate-50 flex flex-wrap items-center justify-between gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="font-semibold text-slate-800">{{ $section->nama }}</h2>
                        <span class="text-xs text-slate-500">{{ $section->members->count() }} anggota · Urutan: {{ $section->urutan }}</span>
                    </div>
                </div>
                <div class="flex items-center gap-2">
                    <button onclick="openMemberModal({{ $section->id }}, '{{ addslashes($section->nama) }}')" class="btn btn-sm btn-primary">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg>
                        Tambah Anggota
                    </button>
                    <button onclick="openEditSectionModal({{ $section->id }}, '{{ addslashes($section->nama) }}', {{ $section->urutan }})" class="btn-icon btn-icon-edit btn-icon-sm">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div class="bg-slate-50 rounded-xl p-4 text-center relative group">
                        ...
                        <div class="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition">
                            <button onclick="openEditMemberModal({{ $member->id }}, '{{ addslashes($member->nama) }}', '{{ addslashes($member->keterangan ?? '') }}', {{ $member->urutan }}, '{{ $member->foto_url ?? '' }}')" class="btn-icon btn-icon-edit btn-icon-sm">
                        ...
                        {{-- Name & Description --}}
                        <h4 class="font-semibold text-slate-800 text-sm">{{ $member->nama }}</h4>
                        @if($member->keterangan)
                        <p class="text-slate-500 text-xs mt-1">{{ $member->keterangan }}</p>
                        @endif
                    </div>
                    @endforeach
                </div>
    ...
    <div class="mt-6 flex justify-end">
        <a href="{{ url('/profil') }}#struktur-organisasi" target="_blank" class="btn btn-outline-secondary btn-lg">
