...
@section('content')
    {{-- Header --}}
    <div class="mb-6">
        <a href="{{ route('admin.pembayaran.index') }}" class="inline-flex items-center text-slate-500 hover:text-[#4276A3]">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-slate-600">Verifikasi pembayaran dari {{ $pembayaran->calonSiswa->nama }}</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
            ...
            <div class="space-y-3 text-sm">
                <div>
                    ...
                    <span class="font-medium text-slate-800">{{ $pembayaran->calonSiswa->nama }}</span>
                </div>
                <div>
                    <span class="block text-slate-500">NISN</span>
                    <span class="font-medium text-slate-800 font-mono">{{ $pembayaran->calonSiswa->nisn }}</span>
                </div>
                <div>
                    <span class="block text-slate-500">Jurusan</span>
                    <span class="font-medium text-slate-800">{{ $pembayaran->calonSiswa->pendaftaran->jurusan->nama ?? '-' }}</span>
                </div>
                <div>
                    <span class="block text-slate-500">Gelombang</span>
                    <span class="font-medium text-slate-800">{{ $pembayaran->calonSiswa->pendaftaran->gelombang ?? '-' }}</span>
                </div>
                <div>
                    <a href="{{ route('admin.pendaftar.show', $pembayaran->calonSiswa->id) }}" class="inline-flex items-center text-[#4276A3] hover:underline text-sm">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
            ...
            <div class="space-y-3 text-sm">
                <div>
                    ...
                    <span class="inline-flex px-2 py-1 text-xs font-medium rounded {{ $statusColors[$pembayaran->status] }}">
                ...
                <div>
                    ...
                    <span class="font-medium text-slate-800">{{ $pembayaran->created_at?->format('d M Y H:i') ?? '-' }}</span>
                </div>
                @if($pembayaran->verified_at)
                <div>
                    <span class="block text-slate-500">Tanggal Verifikasi</span>
                    <span class="font-medium text-slate-800">{{ $pembayaran->verified_at?->format('d M Y H:i') }}</span>
                </div>
                @endif
                @if($pembayaran->catatan_admin)
                <div>
                    <span class="block text-slate-500">Catatan Admin</span>
                    <p class="font-medium text-slate-800">{{ $pembayaran->catatan_admin }}</p>
                </div>
                @endif
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
            ...
            <div class="space-y-4">
                ...
                <form action="{{ route('admin.pembayaran.verify', $pembayaran->id) }}" method="POST" class="space-y-3">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="flex gap-3">
                        <button type="submit" name="status" value="{{ Pembayaran::STATUS_VERIFIED }}" class="flex-1 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 flex items-center justify-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                            </svg>
                            Terima
                        </button>
                        <button type="submit" name="status" value="{{ Pembayaran::STATUS_REJECTED }}" class="flex-1 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 flex items-center justify-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @elseif($pembayaran->status == Pembayaran::STATUS_VERIFIED)
            ...
            @elseif($pembayaran->status == Pembayaran::STATUS_REJECTED)
            <div class="p-4 bg-red-50 rounded-lg border border-red-100">
                ...
                <div class="mt-3 p-3 bg-red-100 rounded">
                    ...
                    <p class="text-sm text-red-700 mt-1">{{ $pembayaran->catatan_admin }}</p>
                </div>
                @endif
            </div>
            @endif
    ...
    <div class="mt-6 bg-white rounded-xl shadow-sm border border-slate-100 p-6">
        ...
        <div class="max-w-lg">
            <img src="{{ route('admin.pembayaran.preview', $pembayaran->id) }}" alt="Bukti Pembayaran" class="w-full rounded-lg border">
        </div>
        <div class="mt-4 flex gap-3">
            <a href="{{ route('admin.pembayaran.preview', $pembayaran->id) }}" target="_blank" class="inline-flex items-center px-4 py-2 border border-slate-300 text-slate-700 rounded-lg hover:bg-slate-50">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('admin.pembayaran.download', $pembayaran->id) }}" class="inline-flex items-center px-4 py-2 bg-[#4276A3] text-white rounded-lg hover:bg-[#3a6a94]">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
