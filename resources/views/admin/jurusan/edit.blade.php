...
@section('title', 'Edit Program Keahlian - Admin Panel')
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-8">
        ...
        <p class="text-slate-600">Ubah data jurusan {{ $jurusan->nama }}.</p>
    </div>

    {{-- Form --}}
    <form action="{{ route('admin.jurusan.update', $jurusan) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="grid md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-300 border-dashed rounded-xl hover:border-blue-400 transition cursor-pointer relative" onclick="document.getElementById('logo').click()">
                        <div class="space-y-1 text-center {{ $jurusan->logo_url ? 'hidden' : '' }}">
                ...
                <div>
                    ...
                    <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-300 border-dashed rounded-xl hover:border-blue-400 transition cursor-pointer relative" onclick="document.getElementById('gambar').click()">
                        <div class="space-y-1 text-center {{ $jurusan->gambar_url ? 'hidden' : '' }}">
                            <svg class="mx-auto h-12 w-12 text-slate-400" stroke="currentColor" fill="none" viewBox="0 0 48 48">
                                <path d="M28 8H12a4 4 0 00-4 4v20m32-12v8m0 0v8a4 4 0 01-4 4H12a4 4 0 01-4-4v-4m32-4l-3.172-3.172a4 4 0 00-5.656 0L28 28M8 32l9.172-9.172a4 4 0 015.656 0L28 28m0 0l4 4m4-24h8m-4-4v8m-12 4h.02" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                            </svg>
                            <div class="flex text-sm text-slate-600 justify-center">
                ...
                @if($jurusan->kompetensiItems->count() > 0)
                    ...
                    <div class="detail-item-kompetensi border border-slate-200 rounded-lg p-3">
                        <div class="flex justify-between items-start mb-2">
                            <input type="text" name="kompetensi_judul[]" value="{{ $item->judul }}"
                        ...
                        <textarea name="kompetensi_deskripsi[]" rows="2" 
                            ...
                            placeholder="Deskripsi kompetensi (opsional)...">{{ $item->deskripsi }}</textarea>
                    </div>
                    @endforeach
                @else
                    {{-- Default item --}}
            ...
            <div id="mapel-container" class="space-y-3">
                @if($jurusan->mapelItems->count() > 0)
                    ...
                    <div class="detail-item-mapel border border-slate-200 rounded-lg p-3">
                        <div class="flex justify-between items-start mb-2">
                            <input type="text" name="mapel_judul[]" value="{{ $item->judul }}"
                                class="flex-1 rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500 text-sm font-medium"
                        ...
                        <textarea name="mapel_deskripsi[]" rows="2" 
                            class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500 text-sm"
                            placeholder="Deskripsi mata pelajaran (opsional)...">{{ $item->deskripsi }}</textarea>
                    </div>
                    @endforeach
                @else
                    {{-- Default item --}}
            ...
            <div id="info-program-container" class="space-y-3">
                @if($jurusan->infoProgram->count() > 0)
                    ...
                    <div class="flex gap-3 info-program-item">
                        <div class="flex-1">
                            <input type="text" name="info_label[]" value="{{ $info->label }}" 
                                class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500 text-sm"
                                placeholder="Label (contoh: Durasi)">
                        </div>
                        <div class="flex-1">
                            <input type="text" name="info_value[]" value="{{ $info->value }}" 
                                class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500 text-sm"
            ...
            <div id="karir-container" class="space-y-3">
                @if($jurusan->karirItems->count() > 0)
                    ...
                    <div class="detail-item-karir border border-slate-200 rounded-lg p-3">
                        <div class="flex justify-between items-start mb-2">
                            <input type="text" name="karir_judul[]" value="{{ $item->judul }}"
                                class="flex-1 rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500 text-sm font-medium"
                        ...
                        <textarea name="karir_deskripsi[]" rows="2" 
                            class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500 text-sm"
                            placeholder="Deskripsi karir (opsional)...">{{ $item->deskripsi }}</textarea>
                    </div>
                    @endforeach
                @else
                    {{-- Default item --}}
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            ...
            <div id="kegiatan-container" class="space-y-4">
                @if($jurusan->kegiatan->count() > 0)
                    ...
                    <div class="kegiatan-item border border-slate-200 rounded-xl p-4" data-index="{{ $index }}">
                        <input type="hidden" name="kegiatan_id[]" value="{{ $kegiatan->id }}">
                        <div class="flex justify-between items-start mb-3">
                            <input type="text" name="kegiatan_judul[]" value="{{ $kegiatan->judul }}"
                        ...
                        <textarea name="kegiatan_deskripsi[]" rows="2" 
                            ...
                            placeholder="Deskripsi kegiatan (opsional)...">{{ $kegiatan->deskripsi }}</textarea>
                        ...
                        <div class="mb-2">
                            ...
                            <div class="flex flex-wrap gap-2 mb-2">
                                ...
                                <div class="relative group" id="kegiatan-gambar-{{ $gambar->id }}">
                                    <img src="{{ $gambar->gambar_url }}" class="w-20 h-20 object-cover rounded-lg border border-slate-200">
                                    <button type="button" onclick="deleteKegiatanGambar({{ $gambar->id }})" 
...
function deleteKegiatanGambar(gambarId) {
    ...
    fetch(`{{ url('admin/jurusan/kegiatan-gambar') }}/${gambarId}`, {
        method: 'DELETE',
        headers: {
