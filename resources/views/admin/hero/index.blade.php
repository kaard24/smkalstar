            ...
            <table class="w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors">
                        <td class="px-4 py-3">
                            <div class="w-20 h-12 rounded overflow-hidden bg-slate-100">
                                @if(!empty($hero->hero_image))
                                    <img src="{{ asset($hero->hero_image) }}" alt="Hero" class="w-full h-full object-cover">
                                @else
                        ...
                        <td class="px-4 py-3">
                            <div class="text-xs text-slate-500">{{ $hero->badge_text }}</div>
                            <div class="font-medium text-slate-800">{{ $hero->title_line1 }} {{ $hero->title_highlight }} {{ $hero->title_line2 }}</div>
                        </td>
                        <td class="px-4 py-3">
                            <div class="text-xs text-slate-500">Dibuat: {{ $hero->creator->name ?? '-' }}</div>
                            <div class="text-xs text-slate-500">Diubah: {{ $hero->updater->name ?? '-' }}</div>
                        </td>
                        <td class="px-4 py-3">
                            @if($hero->is_active)
                        ...
                        <td class="px-4 py-3 text-right">
                            <div class="flex items-center justify-end gap-2">
                                @if(!$hero->is_active)
                                <form action="{{ route('admin.hero.setActive', $hero) }}" method="POST" class="inline">
                                    @csrf
                                    @method('PATCH')
                                ...
                                @endif
                                <a href="{{ route('admin.hero.edit', $hero) }}" class="text-blue-600 hover:text-blue-800 p-1" title="Edit">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                    </svg>
                                </a>
                                <form action="{{ route('admin.hero.destroy', $hero) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus hero section ini?')">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="5" class="px-4 py-8 text-center text-slate-500">
                            <div class="flex flex-col items-center gap-2">
                                ...
                                <a href="{{ route('admin.hero.create') }}" class="text-blue-600 hover:text-blue-800 text-sm">Buat Hero Section</a>
                            </div>
                        </td>
                    </tr>
                    @endforelse
