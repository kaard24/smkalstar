...
@section('title', 'Tambah Hero SPMB - Admin Panel')
...
<div class="max-w-4xl mx-auto">
    ...
    {{-- Form --}}
    <form action="{{ route('admin.spmb.hero.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            ...
            <div class="grid md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="badge_text" value="{{ old('badge_text', 'Pendaftaran Dibuka!') }}" 
                ...
                <div>
                    ...
                    <select name="badge_warna" class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500">
                        <option value="blue" {{ old('badge_warna') == 'blue' ? 'selected' : '' }}>Biru (Blue)</option>
                        <option value="green" {{ old('badge_warna') == 'green' ? 'selected' : '' }}>Hijau (Green)</option>
                        <option value="orange" {{ old('badge_warna') == 'orange' ? 'selected' : '' }}>Oranye (Orange)</option>
                        <option value="purple" {{ old('badge_warna') == 'purple' ? 'selected' : '' }}>Ungu (Purple)</option>
                        <option value="red" {{ old('badge_warna') == 'red' ? 'selected' : '' }}>Merah (Red)</option>
                        <option value="indigo" {{ old('badge_warna') == 'indigo' ? 'selected' : '' }}>Indigo</option>
                    </select>
                    @error('badge_warna')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="judul_baris1" value="{{ old('judul_baris1', 'Sistem Penerimaan') }}" 
                        class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500"
                ...
                <div>
                    ...
                    <input type="text" name="judul_baris2" value="{{ old('judul_baris2', 'Murid Baru 2026/2027') }}" 
                        class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500"
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            ...
            <div>
                <textarea name="deskripsi" rows="3" 
                    class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500"
                    placeholder="Deskripsi singkat tentang SPMB...">{{ old('deskripsi', 'Informasi lengkap mengenai pendaftaran, jadwal, persyaratan, dan biaya pendidikan di SMK Al-Hidayah Lestari') }}</textarea>
                @error('deskripsi')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            ...
            <div class="grid md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="jumlah_gelombang_tampil" value="{{ old('jumlah_gelombang_tampil', 2) }}" min="1" max="5"
                        class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500">
            ...
            <div class="mt-4 space-y-3">
                ...
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="tampilkan_gelombang" value="1" {{ old('tampilkan_gelombang', true) ? 'checked' : '' }}
                        class="rounded border-slate-300 text-blue-600 focus:ring-blue-500">
        ...
        {{-- Background Section --}}
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6" x-data="{ bgType: '{{ old('bg_type', 'default') }}' }">
            <h2 class="text-lg font-semibold text-slate-800 mb-4 flex items-center gap-2">
                <svg class="w-5 h-5 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <select name="bg_type" x-model="bgType" class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500">
                        <option value="default" {{ old('bg_type') == 'default' ? 'selected' : '' }}>Default (Gradien Putih-Biru)</option>
                        <option value="color" {{ old('bg_type') == 'color' ? 'selected' : '' }}>Warna Solid</option>
                        <option value="image" {{ old('bg_type') == 'image' ? 'selected' : '' }}>Gambar</option>
                    </select>
                ...
                <div x-show="bgType === 'color'" x-transition>
                    ...
                    <div class="flex items-center gap-3">
                        <input type="color" name="bg_value" value="{{ old('bg_value', '#ffffff') }}" 
                            class="h-10 w-20 rounded border-slate-300">
                        <input type="text" name="bg_value" value="{{ old('bg_value', '#ffffff') }}" 
