...
<div class="max-w-4xl mx-auto">
    ...
    <form action="{{ route('admin.spmb.hero.update', $hero) }}" method="POST" class="space-y-6">
        ...
        {{-- Background Section --}}
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6" x-data="{ bgType: '{{ old('bg_type', $hero->bg_type ?? 'default') }}' }">
            <h2 class="text-lg font-semibold text-slate-800 mb-4 flex items-center gap-2">
                <svg class="w-5 h-5 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <select name="bg_type" x-model="bgType" class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500">
                        <option value="default" {{ old('bg_type', $hero->bg_type) == 'default' ? 'selected' : '' }}>Default (Gradien Putih-Biru)</option>
                        <option value="color" {{ old('bg_type', $hero->bg_type) == 'color' ? 'selected' : '' }}>Warna Solid</option>
                        <option value="image" {{ old('bg_type', $hero->bg_type) == 'image' ? 'selected' : '' }}>Gambar</option>
                    </select>
                ...
                <div x-show="bgType === 'color'" x-transition>
                    ...
                    <div class="flex items-center gap-3">
                        <input type="color" name="bg_value" value="{{ old('bg_value', $hero->bg_value ?? '#ffffff') }}" 
                            class="h-10 w-20 rounded border-slate-300">
                        <input type="text" name="bg_value" value="{{ old('bg_value', $hero->bg_value ?? '#ffffff') }}" 
                ...
                <div x-show="bgType === 'image'" x-transition>
                    ...
                    {{-- Preview gambar lama --}}
                    @if($hero->bg_value && $hero->bg_type === 'image')
                    <div class="mb-3">
                        <p class="text-xs text-slate-500 mb-1">Gambar saat ini:</p>
                        <img src="{{ asset($hero->bg_value) }}" alt="Background" class="h-32 rounded-lg object-cover border">
                    </div>
                    @endif
