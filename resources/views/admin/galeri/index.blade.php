...
<div class="max-w-6xl mx-auto">
    <div class="mb-8 flex flex-wrap items-center justify-between gap-4">
        ...
        <a href="{{ route('admin.galeri.create') }}" class="btn btn-primary shadow-md hover:shadow-lg">
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden group relative">
            ...
            <div class="p-3 border-t border-slate-100">
                <p class="text-sm text-slate-700 line-clamp-2">{{ $item->keterangan }}</p>
            </div>
            @endif
            
            {{-- Actions Overlay --}}
            <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                <a href="{{ route('admin.galeri.edit', $item->id) }}" class="btn-icon btn-icon-edit">
    ...
    <div class="mt-6 flex justify-end">
        <a href="{{ url('/galeri') }}" target="_blank" class="btn btn-outline-secondary btn-lg">
