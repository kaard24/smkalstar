...
@section('title', isset($fasilitas) ? 'Edit Fasilitas' : 'Tambah Fasilitas' . ' - Admin Panel')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="mb-8">
        <a href="{{ route('admin.fasilitas.index') }}" class="inline-flex items-center text-sm text-gray-500 hover:text-primary mb-4">
            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
            </svg>
            Kembali
        </a>
        <h1 class="text-2xl font-bold text-gray-900">{{ isset($fasilitas) ? 'Edit Fasilitas' : 'Tambah Fasilitas Baru' }}</h1>
    </div>
    ...
    <form action="{{ isset($fasilitas) ? route('admin.fasilitas.update', $fasilitas) : route('admin.fasilitas.store') }}" 
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 bg-gray-50">
                <h2 class="font-semibold text-gray-900">Informasi Fasilitas</h2>
            </div>
            <div class="p-6 space-y-6">
                <div>
                    <label for="nama" class="block text-sm font-medium text-gray-700 mb-2">Nama Fasilitas <span class="text-red-500">*</span></label>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Galeri Gambar Fasilitas</label>
                    @if(isset($fasilitas) && !empty($fasilitas->gambar_urls))
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
                        @foreach($fasilitas->gambar_urls as $index => $url)
                        <div class="relative group">
                            <img src="{{ $url }}" alt="{{ $fasilitas->nama }} {{ $index + 1 }}" class="w-full h-32 object-cover rounded-lg border" loading="lazy" decoding="async">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="number" id="urutan" name="urutan" value="{{ old('urutan', $fasilitas->urutan ?? 0) }}"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary transition">
                    </div>
                    <div class="flex items-center pt-8">
                        <label class="flex items-center cursor-pointer">
                            <input type="checkbox" name="aktif" value="1" {{ old('aktif', $fasilitas->aktif ?? true) ? 'checked' : '' }}
        ...
        <div class="flex justify-end gap-4">
            <a href="{{ route('admin.fasilitas.index') }}" class="px-6 py-3 border border-gray-300 text-gray-700 rounded-xl font-semibold hover:bg-gray-50 transition">
                Batal
            </a>
