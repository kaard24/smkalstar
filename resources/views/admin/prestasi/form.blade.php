...
<div class="max-w-2xl mx-auto">
    <div class="mb-8">
        <a href="{{ route('admin.prestasi.index') }}" class="inline-flex items-center text-sm text-slate-500 hover:text-[#4276A3] mb-4">
            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
            </svg>
            Kembali
        </a>
        <h1 class="text-2xl font-bold text-slate-800">{{ isset($prestasi) ? 'Edit Prestasi' : 'Tambah Prestasi Baru' }}</h1>
    </div>
    ...
    <form action="{{ isset($prestasi) ? route('admin.prestasi.update', $prestasi) : route('admin.prestasi.store') }}" 
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
                        ...
                        <div class="relative group" id="gambar-{{ $index }}">
                            <img src="{{ $url }}" alt="{{ $prestasi->judul }} {{ $index + 1 }}" class="w-full h-32 object-cover rounded-lg border" loading="lazy" decoding="async">
        ...
        <div class="flex justify-end gap-4">
            <a href="{{ route('admin.prestasi.index') }}" class="btn btn-secondary btn-lg">
                Batal
            </a>
