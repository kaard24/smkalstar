...
@section('title', 'Edit Kepala Jurusan - Admin Panel')
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        <form action="{{ route('admin.kajur.update', $kajur) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')

            <div>
                <label class="block text-sm font-medium text-slate-700 mb-2">Nama Lengkap</label>
                <input type="text" name="nama" value="{{ old('nama', $kajur->nama) }}" 
            ...
            <div>
                ...
                <input type="text" name="username" value="{{ old('username', $kajur->username) }}" 
                    class="w-full rounded-lg border-slate-300 focus:border-[#4276A3] focus:ring-[#4276A3]"
            ...
            <div>
                ...
                <select name="jurusan_id" class="w-full rounded-lg border-slate-300 focus:border-[#4276A3] focus:ring-[#4276A3]" required>
                    ...
                    <option value="{{ $j->id }}" {{ old('jurusan_id', $kajur->jurusan_id) == $j->id ? 'selected' : '' }}>{{ $j->nama }}</option>
                    @endforeach
                </select>
                @error('jurusan_id')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            ...
            <div>
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="aktif" value="1" {{ old('aktif', $kajur->aktif) ? 'checked' : '' }}
